...
详细解释见后面的[牌效率](#牌效率)、[手摸切与安牌显示](#手摸切与安牌显示)
...
```
进张数[改良后的进张数加权均值] 切哪张牌 => 向听前进后的进张数的加权均值 [改良数] [进张牌] [荣和点数] [役种] [是否振听]
```
...
- 门清听牌时，会显示默听和立直的各自荣和点数，立直的荣和点数不考虑一发、里宝等
- 存在高低目的场合会显示加权和率的平均荣和点数
...
- 多家立直/副露时会显示综合危险度
- `[n无筋]` 指该玩家的无筋危险牌的剩余种类数。剩余种类数越少，这些无筋牌就越危险；但是当剩余种类数很小时(<=3)，该玩家更有可能是愚型听牌或振听
